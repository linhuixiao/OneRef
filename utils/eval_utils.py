...
# IoU calculation for validation. This IOU can only be implemented in the single-card version.
def IoU(pred, gt):
    # pred = pred.argmax(1)
...
def trans_vg_eval_val(args, pred_boxes, gt_boxes, pred_mask=None, tgt_mask=None):
    ...
    if args.use_mask_loss:
        ...
        mask_iou_list, I_list, U_list = IoU_v2(src_mask, tgt_mask)
    else:
        # mask_iou_list = torch.tensor([])
        mask_iou_list, I_list, U_list = None, None, None

    return iou, accu, mask_iou_list, I_list, U_list


def trans_vg_eval_test(args, pred_boxes, gt_boxes, pred_mask=None, tgt_mask=None):
    pred_boxes = xywh2xyxy(pred_boxes)
    pred_boxes = torch.clamp(pred_boxes, 0, 1)
    gt_boxes = xywh2xyxy(gt_boxes)
    iou = bbox_iou(pred_boxes, gt_boxes)
    ...
    return accu_num, iou, mask_iou_list, I_list, U_list
